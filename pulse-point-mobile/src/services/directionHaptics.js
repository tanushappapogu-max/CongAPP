 ...
 */
export function analyzeDetection(bbox, { allowReach = true } = {}) {
  ...
  let proximity;
  let distanceM;
  if      (area > 0.22 && allowReach) { proximity = Proximity.REACH; distanceM = 0.35; }
...
export class DirectionHapticEngine {
   ...
   */
  update(bbox, options = {}) {
    const { direction, proximity, distanceM } = analyzeDetection(bbox, options);
